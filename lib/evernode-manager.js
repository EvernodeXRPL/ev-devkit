...
class EvernodeManager {
    ...
    async checkHostRealAliveness(hostAddress, currentTimestamp, timeLimit) {
        const currentLedgerIndex = this.#xrplApi.ledgerIndex;
        ...
        if (endLedgerIndex > 0) {
            const transactionHistory = await hostAccount.getAccountTrx(endLedgerIndex, -1, false);
        ...
        return { uptime: Math.round((currentTimestamp - oldestAliveTimestamp) * 100 / 3600) / 100, aliveness: continuousAliveness };
    }
}
