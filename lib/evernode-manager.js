...
class EvernodeManager {
    ...
    constructor(options = {}) {
        ...
        this.#tenantAddress = options.tenantAddress;
    ...
    async init() {
        ...
        if (this.#tenantAddress && this.#tenantSecret) {
    ...
    async acquire(hostAddress, ownerPubkey, moments, config = {}, contractId = uuid.v4(), instanceImage = "hp.latest-ubt.20.04-njs.16") {
        if (!this.#tenantClient)
            throw "Tenant account is not initialized.";
        ...
        if (moments > 1 && instanceName) {
            const extendRes = await this.extend(hostAddress, moments - 1, instanceName);
            info(`Extending the instance for ${moments - 1} moments. Expiry moment: ${extendRes.expiryMoment}`);
        }
