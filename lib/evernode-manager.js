...
class EvernodeManager {
    ...
    async acquire(moments, hostAddress = null, ownerPubkey = null, config = {}, contractId = uuid.v4(), instanceImage = "hp.latest-ubt.20.04-njs.16") {
        if (!this.#tenantClient)
            throw "Tenant account is not initialized.";
        ...
        if (!ownerPubkey) {
            ...
            keysHex = {
                user_pubkey: keys.publicKey,
                user_secret: keys.privateKey
            }
            info('New user key pair generated', keys);
        ...
        return { ...result.instance, ...keysHex };
    }
