...
class InstanceManager {
    ...
    async checkReadRequestStatus() {
        let output = await this.#hpClient.submitContractReadRequest(bson.serialize({ type: 'status' }));
        const result = bson.deserialize(output);

        if (result.type == "statusReport" && result.type != "ok")
            console.log(`(ledger:)>> Status failed. Reason: ${result.message}`);
        else
            console.log(`(ledger:)>>${result.message}`);
    }

    async checkStatus() {
        let status = await this.#hpClient.getStatus();
        if (Object.keys(status).length !== 0) {
            return {
                auditSuccess: true,
                hpVersion: status.hpVersion,
                ledgerSeqNo: status.ledgerSeqNo,
