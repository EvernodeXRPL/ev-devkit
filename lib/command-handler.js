...
const { CONSTANTS, bundleContract, generateKeys, validateArrayElements, removeDirectorySync, copyDirectorySync } = require("./common");
...
async function clusterCreate(size, contractDirectoryPath, contractBin, options) {
    ...
    try {
        ...
        if (options?.multisig) {
            ...
            let totalWeight = size;
            if (options?.signers) {
                options.signers = JSON.parse(fs.readFileSync(options.signers));
                if (options.signers.length == size && validateArrayElements(options.signers, ['account', 'secret', 'weight']))
                    clusterSpec.signers = options?.signers;
                else {
                    throw 'Invalid Signer list';
                }

                totalWeight = options.signers.map(s => s.weight).reduce(function (a, b) {
                    return a + b;
                });
            }

            if (options?.quorum) {
                if (options.quorum > 0 && options.quorum * size <= totalWeight)
...
# Check if secret.cfg file exists and move it to outer level.
if [[ -f ./secret.cfg ]]; then
    mv ./secret.cfg ../secret.cfg
    echo "Moved secret.cfg file in the outer level."
fi
...
`
            ...
            // NOTE :As we upload the bootstrap contract again, we need add relevant binaries and executable files to the contract bundle.
            copyDirectorySync(ASSETS, primaryNodeBundlePath);

            await clusterMgr.writeSignerSeed(`${primaryNodeBundlePath}/secret.cfg`, primaryNode.pubkey);
            const initBundlePath = await bundleContract(primaryNodeBundlePath, [primaryNode.pubkey], 'bootstrap_contract', userKeys.publicKey, undefined, postInstallScript);
            await instanceMgr.uploadBundle(initBundlePath);
            ...
            await Promise.all(
                result.map(async (node) => {
                    ...
                    await clusterMgr.writeSignerSeed(`${nodeBundlePath}/secret.cfg`, node.pubkey);
            ...
            fs.writeFileSync(clusterFilePath, JSON.stringify(clusterFileContent, null, 4));
