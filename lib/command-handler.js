...
` // exit 1 => Purposely making a bootstrap upgrade failure.
            ...
            const initBundlePath = await bundleContract(primaryNodeBundlePath, hpOverrideCfg, postInstallScript);
            ...
            await Promise.all(
                result.slice(1).map(async (node, i) => {
                    ...
                    const secondaryBundlePath = await bundleContract(nodeBundlePath, hpOverrideCfgs, postInstallScript);
                    try {
        ...
        const hpOverrideCfgBp = {
            ...
            mesh: {
                known_peers: result.map(n => `${n.ip}:${n.peer_port}`)
            }
        };
