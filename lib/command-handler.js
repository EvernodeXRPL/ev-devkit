...
async function hostInfo(options) {
    ...
    try {
        ...
        if (options.output) {
            ...
            const csvHeaders = [
                "Address", "Domain", "RAM", "Storage",
                "CPU Model", "CPU Time", "CPU Cores", "CPU Speed",
                "Sashimono Version", "Country Code",
                "Total Instance Slots", "Available Instance Slots", "Active"
            ];
            fs.writeFileSync(tempFileName, `${csvHeaders.join(',')}\n`, { flag: 'w' });
        }

        evernodeMgr = new EvernodeManager();

        await evernodeMgr.init();
        hostsToSearch.forEach(async hostAddress => {
            ...
            if (host) {
                ...
                if (options.output) {
                    // Convert hostDetails object to CSV format
                    const csvData = [
                        hostDetails.address,
                        hostDetails.domain,
                        hostDetails.ram,
                        hostDetails.storage,
                        hostDetails.cpu.model,
                        hostDetails.cpu.time,
                        hostDetails.cpu.cores,
                        hostDetails.cpu.speed,
                        hostDetails.sashimonoVersion,
                        hostDetails.countryCode,
                        hostDetails.totalInstanceSlots,
                        hostDetails.availableInstanceSlots,
                        hostDetails.active
                    ];

                    // Write the CSV string to the temporary file
                    fs.appendFile(tempFileName, `${csvData.join(',')}\n`, (err) => {
                        if (err) {
                            console.error('Error appending to the CSV file:', err.message);
                        } else {
                            console.log('Data appended successfully to the CSV file.');
                        }
                    });
                }
                log(hostDetails);
