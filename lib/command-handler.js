...
const DEFAULT_LIFE_GAP = 2;
const MAX_LIFE_UPPER_BOUND = 48
...
async function audit(options) {
    ...
    try {
        ...
        const AuditResult = (hostAddress, status, timeAcquire = defaultValue, timeReadRequestResponse = defaultValue, timeContractResponse = defaultValue, hpVersion = defaultValue, ledgerSeqNo = defaultValue, errorMessage = defaultValue, inactiveStatus = null) => {
            return {
        ...
        for (let hostIndex in hostsToAudit) {
            ...
            let timeAcquire, timeContractResponse, timeReadRequestResponse, hpVersion, ledgerSeqNo, status, errorMessage;
            ...
            try {
                ...
                auditResults.push(AuditResult(hostAddress, status, timeAcquire, timeReadRequestResponse, timeContractResponse, hpVersion, ledgerSeqNo, errorMessage, inactiveStatus));
                if (instanceMgr)
...
async function clusterCreate(size, contractDirectoryPath, contractBin, options) {
    ...
    try {
            ...
            switch (options.lifePlan) {
                case LifePlan.RANDOM: {
                    ...
                    if (options?.signerLife)
                        throw 'Random life plan does not require the --signer-life in options.';

                    if (options?.moments)
                        throw 'Random life plan does not require the --moments/-m in options.';

                    if (options?.lifeGap)
                        throw 'Random life plan does not require the --life-gap in options.';

                    if (!options?.minLife)
                        throw 'Random life plan requires the --min-life in options.';
                ...
                case LifePlan.INCREMENTAL: {
                    ...
                    if (options?.maxLife || options?.minLife)
                        throw 'Incremental life plan does not need the --min-life or --max-life in options.';

                    if (options?.signerLife)
                        throw 'Incremental life plan does not require the --signer-life in options.';

                    if (options?.moments)
                        throw 'Incremental life plan does not require the --moments in options.';
        ...
        let result = await clusterMgr.createCluster(options.hostsFilePath);
