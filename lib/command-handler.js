...
async function acquireAndDeploy(contractDirectoryPath, contractBin, contractBinArgs, options) {
...
async function clusterCreate(size, contractDirectoryPath, contractBin, contractBinArgs, options) {
    ...
    try {
        ...
        clusterMgr = new ClusterManager({
            ...
            config: appenv.hpConfigPath ? JSON.parse(fs.readFileSync(appenv.hpConfigPath)) : {}
        });
        ...
        info(`Contract bundle uploaded!`);
        success(`Created the cluster!`, result);
    }
    catch (e) {
