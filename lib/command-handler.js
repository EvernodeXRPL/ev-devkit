...
async function list(options) {
    ...
    if (options.desc && !options.orderBy) {
        error('orderBy option is required to order in descending manor.');
        return;
    }
...
async function extend(instancesFilePath, options) {
    ...
    try {
        ...
        const data = fs.readFileSync(instancesFilePath)
        ...
        await Promise.all(instances.map(async (line, i) => {
            ...
            try {
                let [hostAddress, instanceName] = line.split(":");
...
async function clusterCreate(size, contractDirectoryPath, contractBin, hostsFilePath, options) {
    ...
    try {
        ...
        let result;
        try {
            result = await clusterMgr.createCluster(hostsFilePath);
        }
        catch (e) {
