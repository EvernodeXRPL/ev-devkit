...
async function audit(hostAddress, options) {
    info(`command: audit`);
    ...
    try {
        ...
        // const instanceIp = '45.77.199.188';
        // const instanceUserPort = 26201;
        ...
        console.log('1')
        await instanceMgr.checkStatus();
        await instanceMgr.terminate();

    }
    catch (e) {
        error('Error occurred while acquiring the instance: Can not do the audit', e);
    }
    finally {
        if (evernodeMgr)
            await evernodeMgr.terminate();
    ...
    console.log(hostAddress, options)
}
