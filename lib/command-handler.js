...
async function acquire(host, options) {
    ...
    try {
        ...
        const result = await evernodeMgr.acquire(
            ...
            options.contractId,
            options.image,
            appenv.hpInitCfgPath ? JSON.parse(fs.readFileSync(appenv.hpInitCfgPath)) : {});
...
async function bundle(contractDirectoryPath, instancePublicKey, contractBin, options) {
    ...
    try {
        ...
        const userOverrideCfg = appenv.hpOverrideCfgPath ? JSON.parse(fs.readFileSync(appenv.hpOverrideCfgPath)) : {};
...
async function acquireAndDeploy(contractDirectoryPath, contractBin, host, options) {
    ...
    try {
        ...
        const hpConfig = appenv.hpInitCfgPath ? JSON.parse(fs.readFileSync(appenv.hpInitCfgPath)) : {};
        const userOverrideCfg = appenv.hpOverrideCfgPath ? JSON.parse(fs.readFileSync(appenv.hpOverrideCfgPath)) : {};
