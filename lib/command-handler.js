...
const { CONSTANTS, bundleContract, generateKeys, validateArrayElements, removeDirectorySync, formatTime } = require("./common");
...
async function audit(options) {
    ...
    try {
        if (options.filePath) {
            ...
            hostsToAudit = options.filePath ? JSON.parse(fs.readFileSync(options.filePath)) : [];
        }
        ...
        function AuditResult(hostAddress, auditSuccess, timeAcquire = defaultValue, timeReadRequestResponse = defaultValue, timeContractResponse = defaultValue, hpVersion = defaultValue, ledgerSeqNo = defaultValue, roundTime = defaultValue, errorMessage = defaultValue) {
            return {
                "HOST ADDRESS": hostAddress,
                "STATUS": auditSuccess ? "Success" : "Failed",
                "ACQUISITION DURATION (s)": timeAcquire,
                "READ REQUEST RESPONSE DURATION (s)": timeReadRequestResponse,
                "CONTRACT RESPONSE DURATION (s)": timeContractResponse,
                "HP VERSION": hpVersion,
                "LEDGER SEQ NO": ledgerSeqNo,
                "ROUND TIME (ms)": roundTime,
                "ERROR": errorMessage
            }
        }

        for (hostIndex in hostsToAudit) {
            hostAddress = hostsToAudit[hostIndex];
            let timeTracker = new TimeTracker();
            let timeAcquire, timeContractResponse, timeReadRequestResponse, hpVersion, ledgerSeqNo, roundTime, auditSuccess, errorMessage;
            try {
                ...
                timeTracker.start();
                await instanceMgr.checkReadRequestStatus();
                timeReadRequestResponse = timeTracker.end();

                timeTracker.start();
                let statusResult = await instanceMgr.checkStatus();
                timeContractResponse = timeTracker.end();

                auditSuccess = statusResult.auditSuccess;
                hpVersion = statusResult.hpVersion;
                ledgerSeqNo = statusResult.ledgerSeqNo;
                roundTime = statusResult.roundTime;
                ...
                if (e.reason == 'HOST_INACTIVE') {
                    ...
                    errorMessage = e.reason + ` (Downtime: ${formatTime(downtime)})`;
                } else {
                    errorMessage = e.reason;
                }
                error(`Error occured while auditing ${hostAddress}. Error:`, e);
            }
            finally {
                auditResults.push(new AuditResult(hostAddress, auditSuccess, timeAcquire, timeReadRequestResponse, timeContractResponse, hpVersion, ledgerSeqNo, roundTime, errorMessage));
            }
        }
    }
    catch (e) {
        error('Error occured while auditing. Error:', e);
    }
    finally {
...
async function clusterCreate(size, contractDirectoryPath, contractBin, options) {
    ...
    try {
        ...
        if (options?.hostsFilePath && !fs.existsSync(options.hostsFilePath))
        ...
        if (options?.multisig) {
            ...
            if (options?.quorum) {
                const quorum = parseFloat(options?.quorum);
            ...
            clusterSpec.signerMoments = options?.signerMoments ? parseInt(options?.signerMoments) : clusterSpec.moments;
        }

        clusterMgr = new ClusterManager(clusterSpec);

        await clusterMgr.init();
        const result = await clusterMgr.createCluster(options.hostsFilePath);
...
` // exit 1 => Purposely making a bootstrap upgrade failure.
            ...
            await Promise.all(
                result.slice(1).map(async (node, i) => {
                    ...
                    const secondaryBundlePath = await bundleContract(nodeBundlePath, [primaryNode.pubkey], 'bootstrap_contract', node.userKeys.publicKey, undefined, postInstallScript);
                    try {
        ...
        console.log('Waiting 60 seconds until the nodes are synced...');
        await new Promise(resolve => {
