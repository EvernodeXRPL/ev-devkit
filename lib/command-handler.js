...
# Check if signer.cfg file exists and move it to outer level.
if [[ -f ./signer.cfg ]]; then
    mv ./signer.cfg ../signer.cfg
    echo "Moved signer.cfg file in the outer level."
fi
...
`
            ...
            await clusterMgr.writeSigner(`${primaryNodeBundlePath}/signer.cfg`, primaryNode.pubkey);
            ...
            await Promise.all(
                result.map(async (node) => {
                    ...
                    await clusterMgr.writeSigner(`${nodeBundlePath}/signer.cfg`, node.pubkey);
