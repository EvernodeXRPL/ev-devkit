...
const { ClusterManager, NodeStatus, ClusterOwner } = require('./cluster-manager');
...
`;
            ...
            await Promise.all(
                result.map(async (node, i) => {
                    ...
                    const bundlePath = await bundleContract(nodeBundlePath, hpOverrideCfgs, nodePostInstallScript);
                    try {
                        await nodeInstanceMgr.init();
                        await nodeInstanceMgr.uploadBundle(bundlePath, primaryNode.pubkey === node.pubkey);
