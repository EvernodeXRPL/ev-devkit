...
const ASSETS = `./assets/`;
const SECRET_PATH = `${ASSETS}secret.cfg`
...
exit 0
`
            await clusterMgr.writeSignerSeed(SECRET_PATH, primaryNode.pubkey);
            const initBundlePath = await bundleContract(ASSETS, [primaryNode.pubkey], 'bootstrap_contract', userKeys.publicKey, undefined, postInstallScript);
            await instanceMgr.uploadBundle(initBundlePath);
            fs.unlinkSync(SECRET_PATH);

            // TODO other node deployments are there.
        }
