...
async function acquireAndDeploy(tenantAddress, tenantSecret, contractDirectoryPath, contractBin, contractBinArgs, options) {
    ...
    try {
        ...
        const userKeys = options.user ? await generateKeys(options.user, 'hex') : null;
        const result = await evernodeMgr.acquire(options.moments || 1, options.host, userKeys?.publicKey, options.contractId, options.image);
