...
const { questionSync } = require("./common");
...
async function audit(options) {
    ...
    const errorDescription = {
        ...
        8: 'Connection fails during auditing',
        9: 'TRANSACTION_FAILURE Timeout',
        100: 'Unknown error',
        'N/A': 'Not Applicable'
    };
    const errorMap = {
    ...
    try {
        ...
        const AuditResult = (hostAddress, auditSuccess, timeAcquire = defaultValue, timeReadRequestResponse = defaultValue, timeContractResponse = defaultValue, hpVersion = defaultValue, ledgerSeqNo = defaultValue, errorMessage = defaultValue, inactiveStatus = null) => {
            return {
                "HOST ADDRESS": hostAddress,
                "STATUS": inactiveStatus || auditStatus[auditSuccess],
        ...
        if (evrBalance < totalAmount) {
            ...
            const answer = await questionSync(`It will take ${totalAmount} EVRs to do the Audit. Do you wish to proceed (y/n) ? `);
        ...
        for (let hostIndex in hostsToAudit) {
            ...
            let timeAcquire, timeContractResponse, timeReadRequestResponse, hpVersion, ledgerSeqNo, auditSuccess, errorMessage;
            ...
            try {
                ...
                auditResults.push(AuditResult(hostAddress, auditSuccess, timeAcquire, timeReadRequestResponse, timeContractResponse, hpVersion, ledgerSeqNo, errorMessage, inactiveStatus));
                if (instanceMgr)
