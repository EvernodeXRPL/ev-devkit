        ...
        await Promise.all(Array(chunkSize).fill(0).map(async (v, i) => {
            ...
            try {
                ...
                if (this.#nodes.length > 0) {
                    ...
                    let tempNodes = [];
                    Object.assign(tempNodes, this.#nodes);
                    config.mesh.known_peers = (tempNodes.length > MAX_MEMO_PEER_LIMIT) ?
                        tempNodes.sort(() => Math.random() - 0.5).slice(0, MAX_MEMO_PEER_LIMIT).map(n => `${n.ip}:${n.peer_port}`) :
                        tempNodes.map(n => `${n.ip}:${n.peer_port}`);
