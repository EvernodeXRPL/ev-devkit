...
class ClusterManager {
    ...
    async #createClusterChunk(chunkSize = 1) {
        ...
        preferedHostsArray = this.getPreferredHostList();
    ...
    getPreferredHostList(){
        // Reading from file
        let preferedHostsArray = [];
        try {
            // Read contents of the file
            const data = fs.readFileSync('preferredHosts.txt', 'UTF-8')
          
            // Split the contents by new line
            const allLines = data.split(/\r?\n/);

            // Get the preferred host list
            const preferredHostListlines = allLines.slice(4);
