...
class ClusterManager {
    ...
    constructor(options = {}) {
        ...
        if (this.multisig) this.#quorum = options.quorum || this.#size;
    }
    ...
    async createCluster(preferredHostsFilePath) {
        ...
        if (this.multisig) {
            const result = await this.#evernodeMgr.setSigners(this.#quorum, this.#nodes.length);

            this.#nodes.map((n, i) => {
                this.#signers.push({ node: n.pubkey, ...result[i] })
            });
        }
    ...
    async writeSignerSeed(destination, nodePubkey) {
        const normalizedPath = path.normalize(destination);
        const signer = this.#signers.find(n => n.node === nodePubkey);
        if (signer)
            fs.writeFileSync(normalizedPath, signer.secret);
    }
}
