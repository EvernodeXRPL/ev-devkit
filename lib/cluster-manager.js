...
class ClusterManager {
    ...
    async #extendCluster() {
        ...
        await Promise.all(this.#nodes.map(async (node, i) => {
            ...
            try {
                const life = (node.signer_detail ? (this.#signerMoments + ((i + 1) * 2)) : (this.#moments + ((i - this.#signerCount + 1) * 2)));
                // const life = node.signer_detail ? this.#signerMoments : this.#moments;
                if (life > 1) {
    ...
    async #createClusterChunk(chunkSize = 1, preferredHostAddresses = []) {
        const nodes = [];
        ...
        if (preferredHostAddresses.length > 0) {
            preferredHosts = preferredHostAddresses.map(ph => hosts.find(h => h.address === ph)).filter(h => h);
        }
        else {
        ...
        // Limit the chunk size to chosen hosts length to avoid timeouts.
        await Promise.all(Array(Math.min(chunkSize, preferredHosts.length)).fill(0).map(async (v, i) => {
    ...
    async createCluster(preferredHostsFilePath) {
        ...
        // Reading from file
        let preferredHostAddresses = [];
        if (preferredHostsFilePath)
            preferredHostAddresses = this.getPreferredHostList(preferredHostsFilePath);

        while (targetSize > 0) {
            const nodes = await this.#createClusterChunk(targetSize == this.#size ? 1 : targetSize, preferredHostAddresses);
