...
class ClusterManager {
    ...
    async #extendCluster() {
        ...
        await Promise.all(this.#nodes.map(async (node, i) => {
            ...
            try {
                const life = (node.signer_detail ? (this.#signerMoments + (i * 2)) : (this.#moments + (i - this.#signerCount)));
    ...
    async createCluster(preferredHostsFilePath) {
        ...
        while (targetSize > 0) {
            ...
            const nodes = await this.#createClusterChunk(targetSize == this.#size ? 1 : Math.min(targetSize, 2), preferredHostsFilePath);
