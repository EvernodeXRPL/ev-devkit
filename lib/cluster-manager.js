...
class ClusterManager {
    ...
    async createCluster(preferredHostsFilePath) {
        ...
        while (targetSize > 0) {
            // const nodes = await this.#createClusterChunk(targetSize == this.#size ? 1 : targetSize, preferredHostsFilePath);
            const nodes = await this.#createClusterChunk(targetSize == this.#size ? 1 : Math.min(targetSize, 5), preferredHostsFilePath);
